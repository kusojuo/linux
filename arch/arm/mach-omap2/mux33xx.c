...
static struct omap_mux __initdata am33xx_wkup_muxmodes[] = {
...
/*
 * Originally, it was intended that there be only one mux partition, however,
 * the state of several registers is lost during DeepSleep0, so we put these
 * into a separate partition so their state can be saved/restored when DS0
 * occurs. See erratum 1.0.14 'GMII_SEL and CPSW Related Pad Control Registers:
 * Context of These Registers is Lost During Transitions of PD_PER'.
 */
static struct omap_mux __initdata am33xx_per_muxmodes[] = {
...
{
	int ret;
	ret = omap_mux_init("wkup", 0, AM33XX_CONTROL_PADCONF_MUX_WKUP_PBASE,
		AM33XX_CONTROL_PADCONF_MUX_WKUP_SIZE, am33xx_wkup_muxmodes,
		NULL, board_subset, NULL);
	if (ret < 0)
		return ret;
	return omap_mux_init("per", 0, AM33XX_CONTROL_PADCONF_MUX_PER_PBASE,
		AM33XX_CONTROL_PADCONF_MUX_PER_SIZE, am33xx_per_muxmodes,
		NULL, board_subset, NULL);
}
